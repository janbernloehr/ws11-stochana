...
\nomenclature{$\PP$}{vorhersagbare $\sigma$-Algebra bzw. vorhersagbare
...
\begin{definition}
\label{defn:4.1}
\nomenclature[S]{$\HH^2$}{siehe}
\nomenclature[C]{$X_n\Hto X$}{$X_n\to X$ in $\HH^2$}
\index{$\cH^2$-Norm}
...
% \begin{cor*}
% $\cH^2$ ist für Integranden aus $\bL$ abgeschlossen unter stochastischer
% Integration.\fish
% \end{cor*}
% \begin{proof}
...
Die an den Integrator angepasste Topologie auf $\bPP$ haben wir gerade so
...
Offenbar gilt $\bPP \subset L(X)$ für jedes Semimartingal $X$. Die Klasse $L(X)$
